...
class HAM10000Dataset(Dataset):
	...
	def _load_metadata(data_dir: Path) -> pd.DataFrame:
		...
		missing_items = [str(path) for path in (metadata_csv, part_1, part_2) if not path.exists()]
		if missing_items:
			missing_text = "\n - ".join([""] + missing_items)
...
def create_data_splits(
	...
	train_indices, test_indices = train_test_split(indices, test_size=test_size, random_state=random_state, stratify=labels)
...
def create_dataloaders(
	...
	train_dataset, test_dataset = create_data_splits(dataset, test_size=test_size, random_state=random_state)
...
if __name__ == "__main__":
	...
	train_dataset.dataset.metadata.head().to_csv("metadata_head.csv", index=False)